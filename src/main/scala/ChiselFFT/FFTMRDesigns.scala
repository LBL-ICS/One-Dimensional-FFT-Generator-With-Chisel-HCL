...
object FFTMRDesigns {
  ...
  class FFT_MixedRadix_Streaming_v2(N: Int, nr: Int, ns: Int, r: Int, s: Int, w: Int, bw: Int) extends Module { // streaming single radix fft, still in progress
    val w1 = w // this will be the input and output width
    var w2 = Permutations.getw2(w1, N, nr, ns, s)
    val io = IO(new Bundle() {
      val in = Input(Vec(w, new ComplexNum(bw)))
    ...
    if (w1 < nr && w2 < ns) { //streaming width is less the a single FFTnr size - will have to be streamed as well \\ additionally this can only be the case if nr is greater than fundamental dft2 and 3 sizes
      ...
      fftlatency1 = getfftstreamedlatency(nr, r, w1, bw)
      fftlatency2 = getfftstreamedlatency(ns, s, w2, bw)
      val FFT_modules1 = Module(new FFT_SingleRadix_Streaming_NRO_v2(nr, r, w1, bw)).io
      val FFT_modules2 = Module(new FFT_SingleRadix_Streaming_NRO_v2(ns, s, w2, bw)).io // it seems that we have already accounted for the case where ns > s
      ...
      FFT_modules1.in_en := io.in_en
      FFT_modules2.in_en := io.in_en
      Perm_module1.in_en_main := io.in_en
      Perm_module2.in_en_main := io.in_en
      Perm_module3.in_en_main := io.in_en
      Twid_Modules.in_en_main := io.in_en
      ...
      val FFT_modules2 = if (ns == s) { // it could either be that ns == s
        val row = for (j <- 0 until w2 / ns) yield { // dft modules
          val instance = Module(new DFT_Symmetric_NRV_v2(s, bw)).io
          instance
        }
        row
      } else { // or it is greater
        val row = for (j <- 0 until w2 / ns) yield {
          val instance = Module(new FFT_SingleRadix_NRV_v2(ns, s, ns, bw)).io
          instance
        }
        row
      }
      ...
      val FFT_modules1 = if (nr == r) { // if this this the case then we just need to instantiate dft modules
        ...
        val row = for (j <- 0 until w1 / nr) yield { // since nr != r // w is ideally a multiple of nr // also notice that this is based on the first streaming width
          val instance = Module(new FFT_SingleRadix_NRV_v2(nr, r, nr, bw)).io
          instance
        }
        row
      }
      //same case with the other sets of FFT modules
      val FFT_modules2 = Module(new FFT_SingleRadix_Streaming_NRO_v2(ns, s, w2, bw)).io // the streamed FFT2
      val Perm_module1 = Module(new PermutationsWithStreaming_v2(N, ns, ns, w1, 0, bw)).io
      val Perm_module2 = Module(new PermutationsWithStreaming_mr_v2(N, nr, nr, w2, w1, 0, bw, delay_cycles_stage2)).io
      val Perm_module3 = Module(new PermutationsWithStreaming_mr_v2(N, ns, ns, w1, w2, 0, bw, delay_cycles_stage2)).io
      val Twid_Modules = Module(new TwiddleFactorsStreamed_mr_v2(N, nr, ns, w2, bw, ((N / w1)))).io
      for(i <- 0 until w1/nr) {
        FFT_modules1(i).in_en := io.in_en
      }
      FFT_modules2.in_en := io.in_en
      Perm_module1.in_en_main := io.in_en
      Perm_module2.in_en_main := io.in_en
