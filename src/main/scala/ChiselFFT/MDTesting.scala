...
object MDTesting {
  def main(args: Array[String]):Unit = {
    ...
    val Nd = 5 // the size of each dimension
    ...
    //
    val pw3 = new PrintWriter("FFT3D.v")
    ...
    //      c.io.in_a.poke(convert_string_to_IEEE_754("169.0", 32).U)
    //      for(i <- 0 until 30){
    //        c.clock.step(1)
    //        println(s"clock cycle:${i+1}")
    //        println(s"Output: ${convert_long_to_float(c.io.out_s.peek().litValue, 32)}")
    ...
    //    }
    //    println("converting")
    //    val number =  0.000000000000000001
    //    println(number.toString)
    //    val number_hex = convert_string_to_IEEE_754((number).toString, bw)
    //    println(number_hex.toLong.toHexString)
    //
    //    test(new FP_multiplier(bw)){c=>
    //      c.io.in_a.poke(number_hex.U)
    //      c.io.in_b.poke(number_hex.U)
    //      c.clock.step(1)
    //      println(s"Output: ${convert_long_to_float(c.io.out_s.peek().litValue,bw)}")
    //    }
  }
}
