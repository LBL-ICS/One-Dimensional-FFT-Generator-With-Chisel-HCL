...
//    println("--------")
    test(new DFT_r(4, 32)){c=>
      val CMultLatency = 2
      val CAddLatency = 1
      val DFT_latency = CMultLatency + ((Math.log10(4)/Math.log10(2)).round.toInt + (for(l <- 0 until (Math.log10(4)/Math.log10(2)).round.toInt)yield{(4/Math.pow(2,l)).round.toInt % 2}).reduce(_+_)) * (CAddLatency)
      println(DFT_latency)
      for(i <- 0 until 2) {
        c.io.in_Override.poke(false.B)
        c.io.in_en.poke(true.B)
        c.io.in(0).Re.poke(convert_string_to_IEEE_754("12.3", 32).U)
...
println("-----------------------------------------------??????????????????")
    test(new FFT_sr(4,2,4,32)){c=>
      ...
      for(i <- 0 until Total_Latency) {
        println(s"cond: ${convert_long_to_float(c.io.out_test.peek().Re.litValue, 32)}")
        c.clock.step(1)
        println(s"cond: ${convert_long_to_float(c.io.out_test.peek().Re.litValue, 32)}")
        println(s"Clock cycle ${i+1}")
        println(s"Real Output: ${convert_long_to_float(c.io.out(0).Re.peek().litValue, 32)}")
