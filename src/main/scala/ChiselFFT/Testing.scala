...
//    pw3.close()
//    test(new FFT_mr_basic(12,4,3,2,3,12,bw)){c=>
//      c.io.in_ready.poke(true.B)
...
//      }
//    }
//    test(new FFT_sr_v2_streaming_and_iterative2(16,2,1,2,32)){c=> // for now, do not set streaming-width equal to the N-width, also the streaming width must be a multiple of the radx and a divisor of the N. The iterative depth must be a divisor of the total number of stages required for the computation of the fft
//      c.io.in_ready.poke(true.B)
//      c.io.in(0).Re.poke(convert_string_to_IEEE_754("1.0", 32).U)
//      c.io.in(0).Im.poke(convert_string_to_IEEE_754("0.0", 32).U)
//      c.io.in(1).Re.poke(convert_string_to_IEEE_754("2.0", 32).U)
...
////      c.io.in(2).Im.poke(convert_string_to_IEEE_754("0.0", 32).U)
//      c.clock.step(1)
//      c.io.in(0).Re.poke(convert_string_to_IEEE_754("3.0", 32).U)
//      c.io.in(0).Im.poke(convert_string_to_IEEE_754("0.0", 32).U)
//      c.io.in(1).Re.poke(convert_string_to_IEEE_754("4.0", 32).U)
//      c.io.in(1).Im.poke(convert_string_to_IEEE_754("0.0", 32).U)
//      c.clock.step(1)
